...
<template>
  <div v-if="show" class="fixed inset-0 modal-overlay z-50 flex items-center justify-center p-4">
    <div class="bg-[var(--secondary-parchment)] rounded-lg border-[var(--primary-gold)] border-2 max-w-2xl w-full max-h-96 overflow-y-auto">
      <div class="p-6 border-b border-gray-200">
        ...
        <div class="mt-4 flex space-x-2 text-2xl">
          <input
            type="text"
            v-model="searchTerm"
            placeholder="Search vocabulary..."
            class="text-black flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent"
          />
          <!-- <button
      ...
      <div class="p-6 bg-[var(--oxford-blue)]">
        ...
        <div v-else>
          ...
          <div class="grid grid-cols-4 gap-3">
            ...
            >
              <div class="font-bold text-2xl">{{ entry.word }}</div>
            </div>
          </div>
        </div>
      </div>
