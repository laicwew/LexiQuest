...
describe('AI Console Functionality', () => {
  ...
  it('should clear game history when clearGameHistory is called', () => {
    ...
    expect(store.generatedContent).toBe("You are in a <span class=\"interactive-word\" data-word=\"forest\">forest</span>.")
  ...
  it('should append new content when there is game history', () => {
    ...
    // 模拟新生成的内容
    const newContent = "You decide to walk deeper into the forest. The trees seem to close in around you."
    ...
    if (store.gameHistory.length > 0) {
      ...
      const expectedContent = currentContent ? currentContent + separator + processedNewContent : processedNewContent;
      
      expect(expectedContent).toBe("You are in a <span class=\"interactive-word\" data-word=\"forest\">forest</span>.<br><br>---<br><br>You decide to walk deeper into the forest. The trees seem to close in around you.")
    }
  })
})